...
def _dashboard_load(path, default):
    if path == "market_data.json" and os.environ.get("SDA_REMOTE_STATE") == "1":
        ...
        if isinstance(full_market, dict) and isinstance(compact_market, dict):
            full_tokens = full_market.get("tokens")
            compact_tokens = compact_market.get("tokens")
            if isinstance(full_tokens, dict) and isinstance(compact_tokens, dict):
                merged = dict(full_market)
                merged_tokens = dict(full_tokens)
                merged_tokens.update(compact_tokens)
                merged["tokens"] = merged_tokens
                for key, value in compact_market.items():
                    if key != "tokens":
                        merged[key] = value
                return merged
            if isinstance(compact_tokens, dict) and compact_tokens:
