...
Installed after all dashboard UI patches so MAIN/REAL/REAL_STATS/REFRESH
...
def patch_dashboard(dashboard):
    ...
    def _send_fallback(chat_id, text, keyboard=None):
        """Send a fresh Telegram message if the callback message cannot be edited."""
        if not chat_id:
            return
        ...
        try:
            dashboard.api("sendMessage", payload)
        except Exception as exc:
            print(f"Telegram fallback send error: {exc}")

    def _safe_edit(chat_id, message_id, text, keyboard=None):
        """Edit when possible; fall back to a new message on Telegram HTTP 400."""
        ...
        try:
            ...
            print(f"Telegram edit fallback trigger: {exc}")
        _send_fallback(chat_id, text, keyboard)
        return False
    ...
    def handle_update(update, state=None):
        ...
        # These callbacks are final/authoritative here. Everything else keeps
        # the existing dashboard handlers and their legacy behavior.
        if data not in ("MAIN", "REAL", "REAL_STATS", "REFRESH"):
        ...
        if data == "MAIN":
            try:
                ...
                _safe_edit(chat_id, message_id, text, keyboard)
            except Exception as exc:
                _safe_edit(
                    chat_id,
                    message_id,
                    f"⚠️ Main dashboard error: {type(exc).__name__}: {exc}",
        ...
        if data == "REAL_STATS":
            ...
            _safe_edit(chat_id, message_id, text, dashboard.back_keyboard())
            return state

        # REFRESH: explicitly dispatch the hourly report workflow, then render
        # the current main dashboard. This avoids the old nested handler and
        # keeps the refresh button functional even when the original message
        # cannot be edited.
        ...
        try:
            ...
            if ok:
                ...
                text = dashboard.main_dashboard() + f"\n\n❌ Hourly Report start failed: {result}"
        except Exception as exc:
            text = dashboard.main_dashboard() + f"\n\n❌ Refresh error: {type(exc).__name__}: {exc}"
        _safe_edit(chat_id, message_id, text, dashboard.menu_keyboard())
        return state
