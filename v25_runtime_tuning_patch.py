...
MAX-WIN is untouched. V25 remains isolated and paper-only. The learned V25
hunter keeps the strict confirmation lane, plus controlled opportunity lanes
so healthy pump setups can actually generate paper BUY samples.
"""
...
def patch():
    ...
    if original_gate and not getattr(hunter, "_sda_warmup_gate_patched", False):
        def warmup_gate(c, learned):
            ...
            # EARLY-PUMP: explosive move before slower flow/trade confirmation.
            early = (
            ...
            # QUALITY-PUMP: lets the learner participate when expected outcome
            # is positive and downside is controlled, even before a full burst.
            quality = (
    ...
    try:
        ...
        # Keep the canonical MAX-WIN report unchanged, but append the separate
        # V25 paper book so its real open positions are visible.
                ...
                try:
                    ...
                    for address, pos in positions.items():
                        if not isinstance(pos, dict):
                            continue
                        inv = float(pos.get("investment_sda") or 50.0)
                        invested += inv
                        td = tokens.get(str(address).lower(), {})
                        an = dashboard._analysis(td) if isinstance(td, dict) else {}
                        entry = float(pos.get("entry_price") or 0)
                        cur = float(an.get("price_in_sda") or 0)
                        pnl = inv * (cur / entry) * .999 * .99 - inv * 1.01 if entry > 0 and cur > 0 else 0.0
                        open_pnl += pnl
                        roi = (cur / entry - 1) * 100 if entry > 0 and cur > 0 else 0.0
                    ...
                    if not rows:
                        lines.append("⚪ No open V25 positions")
                    else:
