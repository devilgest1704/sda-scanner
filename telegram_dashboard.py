...
def market_debug_report(snapshot=None):
    ...
    if not rows:
        ...
        for i, row in enumerate(rows, 1):
            ...
            gate = "PASS" if (not tech_available or bull >= 2) else "BLOCK"
            ...
            lines.append(
                ...
                f"adjustment {adj:+.1f} | technical gate {gate} | "
                f"technical data {'YES' if tech_available else 'N/A'}"
            )
            if tc["evidence"]:
...
def _resolve_market_token(token, pf, tokens, meta):
    target = (token or "").strip().upper()
    for address, data in tokens.items():
        label = engine.lbl(address, meta).upper()
        if target in {label, address.upper()} or label.startswith(target + "/"):
            return address
    return None
...
def _position_recommendations(md, ws, meta, portfolio):
    ...
    for address, pos in positions.items():
        analysis = _analysis((md.get("tokens", {}) or {}).get(address, {}))
        if not analysis:
            continue
        try:
            s = engine.score(address, analysis, ws)
            ...
            continue
        score = engine.num(s.get("confidence"))
        pnl = pos.get("pnl_sda") if isinstance(pos, dict) else None
        action = "HOLD"
        if score < 45:
            action = "SELL / EXIT"
        elif score < 55:
            action = "PARTIAL SELL"
        result.append({"address": address, "symbol": engine.lbl(address, meta), "score": score, "pnl_sda": pnl, "action": action})
    return result


def _merge_wallet_portfolio(wallet, portfolio, md, ws, meta):
    lines = ["💼 REAL PORTFOLIO", ""]
    positions = portfolio.get("positions", {}) if isinstance(portfolio, dict) else {}
    if not positions:
        return "\n".join(lines + ["⚪ No portfolio positions"])
    for address, pos in positions.items():
        symbol = engine.lbl(address, meta)
        amount = engine.num(pos.get("amount")) if isinstance(pos, dict) else 0
        pnl = pos.get("pnl_sda") if isinstance(pos, dict) else None
        pnl_txt = "UNKNOWN" if pnl is None else f"{engine.num(pnl):+.2f} SDA"
        lines.append(f"🪙 {symbol}")
        lines.append(f"   {amount:,.2f}  •  P/L {pnl_txt}")
    return "\n".join(lines)


def paper_stats():
    stats = load("paper_stats.json", {})
    return "🤖 SDA PAPER TRADING\n\n" + json.dumps(stats, ensure_ascii=False, indent=2)
...
def technical_report():
    ...
    for i, row in enumerate(rows, 1):
        ...
        for key in ("rsi_14", "macd", "macd_signal", "ema_20", "ema_50", "price_in_sda"):
            if key in tech: lines.append(f"   {key}: {tech[key]}")
        lines.append("")
    return "\n".join(lines)


def _handle_callback(q):
    data = (q.get("data") or "").upper()
    cid = q.get("id"); msg = q.get("message") or {}; chat = (msg.get("chat") or {}).get("id"); mid = msg.get("message_id")
    answer_callback(cid)
    if data == "MAIN": edit(chat, mid, main_dashboard(), menu_keyboard())
    elif data == "DEBUG": edit(chat, mid, market_debug_report(), back_keyboard())
    elif data == "MOMENTUM": edit(chat, mid, market_momentum_report(), back_keyboard())
    elif data == "TECH": edit(chat, mid, technical_report(), back_keyboard())
    elif data == "PAPER": edit(chat, mid, paper_stats(), paper_menu_keyboard())
    elif data == "PAPER_STATS": edit(chat, mid, paper_stats(), paper_menu_keyboard())


def main():
    if len(sys.argv) > 1 and sys.argv[1] == "--debug":
        print(market_debug_report())
        return
