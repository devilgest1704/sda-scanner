...
def _buy_gate_rows(md, ws, meta, limit=5):
    ...
    for address, token_data in tokens.items():
        ...
        rows.append({
            ...
            "net_1h": engine.num(s.get("net_1h")),
            "whale_net": engine.num(s.get("whale_net")),
            "reasons": reasons,
        })
...
def market_debug_report():
    ...
    for token_data in tokens.values():
        ...
        if vol >= 250:
            active += 1
    lines = [
        "🐞 MARKET DEBUG", "", f"Loaded tokens: {loaded}", f"Analyzed tokens: {analyzed}",
        f"Active tokens: {active}", f"1H volume total: {total_volume:.0f} SDA",
        f"1H trades total: {total_trades:.0f}", f"Whale data entries: {len(ws) if isinstance(ws, dict) else 0}",
        "", "Active rule: 1H volume ≥ 250 SDA", "Trades do not control activity filtering",
        "", "🎯 TOP BUY CANDIDATE DIAGNOSTICS", f"BUY threshold: {engine.BUY_THRESHOLD}/100",
...
def _position_recommendations(md, ws, meta, portfolio):
    ...
    for token, pf in cur.items():
        ...
        score = engine.num(s.get("confidence")); m1h = engine.num(s.get("m1h")); flow = engine.num(s.get("net_1h"))
        pnl_raw = pf.get("unrealized_pnl_pct"); pnl = engine.num(pnl_raw)
        old = state.get(token, {}) if isinstance(state.get(token), dict) else {}
        neg = int(engine.num(old.get("negative_count"))); weak = int(engine.num(old.get("profit_weakening_count"))); loss_weak = int(engine.num(old.get("loss_weakening_count")))
