...
def _gate_reasons(d):
    try:
        ...
        data=(d or {}).get("data") or {};an=data.get("analysis") if isinstance(data.get("analysis"),dict) else data;flow=an.get("flow",{}).get("1h",{}) if isinstance(an,dict) else {};score=_num((d or {}).get("score",data.get("buy_score",data.get("confidence"))));volume=_num(data.get("volume_1h",an.get("volume_1h") if isinstance(an,dict) else 0));trades=_num(data.get("trades_1h")) or _num(flow.get("buy_count"))+_num(flow.get("sell_count"));bear=int((d or {}).get("technical_bear",data.get("technical_bear")) or 0)
...
def patch_dashboard(dashboard):
    ...
    def debug_v25(snapshot=None):
        ...
        for i,r in enumerate(rows,1):
            ...
            lines += [f"{i}. {icon} • {'🟢 CORE' if not r['decision'].get('blocked') else '🔴 CORE BLOCK'} • {r['label']} • score {r['score']:.0f}/100",f"   V25 EV {r['v25_expected_pl']:+.2f} SDA • MFE {r['v25_expected_mfe']:+.1f}% • MAE {r['v25_expected_mae']:+.1f}% • P10/P20/P30 {r['v25_p10']:.0%}/{r['v25_p20']:.0%}/{r['v25_p30']:.0%}",f"   Gate: {gate}"]
        return "\n".join(lines)
