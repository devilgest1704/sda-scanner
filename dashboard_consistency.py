...
def patch_dashboard(dashboard):
    ...
    def _wallet_synced_portfolio(wallet, portfolio):
        ...
        allowed_symbols = {
            ...
            for h in wallet.get("holdings", []) or [] if isinstance(h, dict)
        }
        allowed_addresses = {
    ...
    def _paper_row(address, analysis, ws, meta, label=None):
        """Score through main.paper_decision(), never through legacy engine.score()."""
        try:
            import main as scanner
            decision = scanner.paper_decision(address, analysis, ws)
        except Exception as exc:
            return {
                "score": None, "blocked": True, "reason": f"paper scorer error: {exc}",
                "prediction": {}, "prediction_blocked": True, "prediction_text": "",
                "technical_bull": 0, "technical_bear": 0, "technical_evidence": [],
                "components": {}, "raw_confidence": None, "near_threshold": False,
                "near_threshold_reason": "", "score_band": "", "data": {},
            }
        data = decision.get("data") or {}
        return {
            **decision,
            "label": label or engine.lbl(address, meta),
            "m1h": engine.num(data.get("m1h")) if data.get("m1h") is not None else None,
            "m4h": engine.num(data.get("m4h")) if data.get("m4h") is not None else None,
            "m15": engine.num(data.get("m15")) if data.get("m15") is not None else None,
            "flow_1h": engine.num(data.get("net_1h")) if data.get("net_1h") is not None else None,
            "trades_1h": engine.num(data.get("trades_1h")) if data.get("trades_1h") is not None else None,
        }
    ...
    def _canonical_recommendations(md, ws, meta, portfolio):
        ...
        for token, pf in current.items():
            ...
            decision = _paper_row(market_key or token, analysis, ws, meta) if analysis else {
                "score": None, "blocked": True, "reason": "market data not resolved", "data": {}
            }
            data = decision.get("data") or {}
            score = engine.num(decision.get("score")) if decision.get("score") is not None else None
            m1h = decision.get("m1h")
            flow1 = decision.get("flow_1h")
            ...
            try:
                ...
                exit_signal = evaluate_exit(
                    pnl if pnl is not None else 0.0, score or 0.0,
                    m1h or 0.0, flow1 or 0.0, bearish,
                )
            except Exception:
                bearish = False
                exit_signal = {}
            ...
            if exit_signal.get("emergency"):
                action, reason = "EMERGENCY SELL", "ROI ≤ -15% with weak score, momentum and SDA flow"
                ...
                action, reason = "SELL / EXIT", "loss > 8% + weak score + negative momentum + SDA flow"
            elif pnl is not None and pnl <= -4 and negative_evidence >= 3:
                action, reason = "SELL / EXIT", "loss > 4% with 3+ bearish signals"
                ...
                action, reason = "PARTIAL SELL", "profit > 5% but trend/flow is weakening"
            elif pnl is not None and pnl > 10 and score is not None and score < 45 and negative_evidence >= 2:
                action, reason = "PARTIAL SELL", "profit > 10% with weakening market evidence"
            elif pnl is not None and pnl > 0 and positive_evidence >= 3 and score is not None and score >= 55:
                action, reason = "HOLD / TRAIL", "position profitable with supportive trend/flow"
            elif pnl is not None and pnl < 0 and negative_evidence >= 2:
                action, reason = "HOLD / WATCH", "loss with mixed-to-bearish evidence; monitor next scan"
            elif data_missing:
                action, reason = "HOLD / MARKET DATA N/A", "market data not resolved for this wallet token"
            else:
                action, reason = "HOLD / WATCH", "no confirmed exit condition"
            rows.append({
                "token": token, "address": market_key, "symbol": pf.get("symbol") or engine.lbl(market_key or token, meta),
                "action": action, "reason": reason, "pnl_pct": pnl_raw,
                "pnl_sda": pf.get("unrealized_pnl_sda"), "score": score,
                "m1h": m1h, "flow_1h": flow1, "paper_blocked": bool(decision.get("blocked")),
                "paper_reason": decision.get("reason") or "", "paper_band": decision.get("score_band") or "",
            })
        return sorted(rows, key=lambda x: (
            x.get("pnl_sda") is None, -engine.num(x.get("pnl_sda")), str(x.get("symbol") or "").upper()
        ))

    def _canonical_buy_gate_rows(md, ws, meta, limit=5):
        """TOP BUY rows use exactly the same Paper scorer as the real scanner."""
        rows = []
        tokens = md.get("tokens", {}) or {}
        threshold = float(getattr(engine, "BUY_THRESHOLD", 60) or 60)
        min_trades = float(getattr(engine, "MIN_TRADES_1H", 3) or 3)
        for address, token_data in tokens.items():
            ...
            if volume_1h < 250:
                continue
            d = _paper_row(address, analysis, ws, meta)
            score = engine.num(d.get("score")) if d.get("score") is not None else None
            reasons = []
            if score is None:
                reasons.append(d.get("reason") or "paper score unavailable")
            elif d.get("blocked"):
                reasons.append(d.get("reason") or "paper BUY veto")
            elif score < threshold:
                reasons.append(f"BUY score {score:.0f}<{threshold:.0f}")
            if trades < min_trades:
                reasons.append(f"trades {trades:.0f}<{min_trades:.0f}")
            if not reasons:
                reasons.append("ALL PAPER BUY GATES PASS")
            rows.append({
                "address": address, "label": engine.lbl(address, meta), "analysis": analysis,
                "score_data": d.get("data") or {}, "score": score or 0.0,
                "trades": trades, "volume_1h": volume_1h,
                "m1h": d.get("m1h"), "m4h": d.get("m4h"), "m15": d.get("m15"),
                "net_1h": d.get("flow_1h"), "whale_net": engine.num((d.get("data") or {}).get("whale_net")),
                "reasons": reasons, "paper": d,
            })
    ...
    def real_trading_report_canonical():
        ...
        if not rows:
            ...
            for row in rows:
                ...
                lines.append(f"{_action_icon(row.get('action'))} {row.get('symbol')}: {row.get('action')} • P/L {pnl} • score {score}")
                lines.append(f"   {row.get('reason')}")
        lines += ["", "────────────────────────", "👁 READ-ONLY • No real order is executed"]
        return "\n".join(lines)

    def market_debug_report_canonical(snapshot=None):
        """Debug the actual Paper BUY path, not the legacy diagnostic score."""
        if snapshot is None:
            md = dashboard.load("market_data.json", {"tokens": {}})
            ws = dashboard.load("whale_data.json", {})
            meta = dashboard.load("token_metadata.json", {})
            rows = _canonical_buy_gate_rows(md, ws, meta, 5)
            snapshot = {"md": md, "ws": ws, "meta": meta, "rows": rows, "id": dashboard._snapshot_id(md, ws, meta), "time": dashboard._snapshot_time(md, ws)}
        rows = snapshot["rows"]
        md = snapshot["md"]
        ws = snapshot["ws"]
        tokens = md.get("tokens", {}) or {}
        ...
        for td in tokens.values():
            ...
            vol = engine.num((f or {}).get("total_volume"))
            total_volume += vol
            total_trades += engine.num((f or {}).get("buy_count")) + engine.num((f or {}).get("sell_count"))
            if vol >= 250:
                active += 1
        threshold = float(getattr(engine, "BUY_THRESHOLD", 60) or 60)
        ready = sum(1 for r in rows if not r["reasons"] or r["reasons"] == ["ALL PAPER BUY GATES PASS"])
        near = sum(1 for r in rows if r.get("paper", {}).get("near_threshold"))
        lines = [
            "🐞 MARKET DEBUG • CANONICAL PAPER PATH", "",
            f"Snapshot: {snapshot['id']}", f"State time: {snapshot['time']}",
            f"Loaded tokens: {len(tokens)}", f"Analyzed tokens: {analyzed}", f"Active tokens: {active}",
            f"1H volume total: {total_volume:.0f} SDA", f"1H trades total: {total_trades:.0f}",
            f"Whale data entries: {len(ws) if isinstance(ws, dict) else 0}",
            "", f"Active filter: 1H volume ≥ 250 SDA", f"BUY threshold: {threshold:.0f}/100", "Trade count: informational gate",
            "────────────────────────", f"🟢 PAPER BUY READY in TOP {len(rows)}: {ready}", f"🟡 BORDERLINE/BRIDGE: {near}",
            "", "🎯 TOP PAPER BUY DIAGNOSTICS", "────────────────────────",
        ]
        if not rows:
        ...
        for i, row in enumerate(rows, 1):
            ...
            status = "🟢 BUY READY" if not row["reasons"] or row["reasons"] == ["ALL PAPER BUY GATES PASS"] else "🔴 BLOCKED"
            lines.append(f"{i}. {status} {row['label']} — {row['score']:.1f}/100")
            lines.append(f"   V2: momentum {comp.get('momentum','N/A')} | flow {comp.get('flow','N/A')} | activity {comp.get('activity','N/A')} | prediction {comp.get('prediction','N/A')} | liquidity {comp.get('liquidity','N/A')}")
            lines.append(f"   INPUT: 1H {row.get('m1h',0):+.2f}% | 4H {row.get('m4h',0):+.2f}% | 15M {row.get('m15',0):+.2f}% | volume {row['volume_1h']:.0f} | trades {row['trades']:.0f}")
            lines.append(f"   TECH: bull {p.get('technical_bull',0)} | bear {p.get('technical_bear',0)} | adjustment {engine.num((p.get('data') or {}).get('v21_adjustment')):+.1f}")
            if pred:
                lines.append(f"   PREDICTOR: {'READY' if pred.get('ready') else 'WARMING'} | P(+5) {engine.num(pred.get('p5')):.0%} | P(+10) {engine.num(pred.get('p10')):.0%} | mean {engine.num(pred.get('mean_roi')):+.1f}%")
            else:
                lines.append("   PREDICTOR: N/A")
            if p.get("reason"):
                lines.append(f"   BLOCK: {p.get('reason')}")
            elif row["reasons"] != ["ALL PAPER BUY GATES PASS"]:
                lines.append(f"   BLOCK: {'; '.join(row['reasons'])}")
            else:
                lines.append("   FINAL: PAPER BUY READY")
        return "\n".join(lines)
