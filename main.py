...
def whale(addr,ws,w):
    key = str(addr).lower()
    x = ws.get(key, {}) if isinstance(ws, dict) else {}
    if not isinstance(x, dict):
        return {}

    # Current rolling schema.
    ...
    # Legacy cumulative schema fallback.
    if w == "1h":
        return {
            "buy_volume": num(x.get("whale_buy_volume")),
            "sell_volume": num(x.get("whale_sell_volume")),
            "net_flow": num(x.get("whale_buy_volume")) - num(x.get("whale_sell_volume")),
            "buy_count": num(x.get("whale_buy_count")),
            "sell_count": num(x.get("whale_sell_count")),
            "trade_count": num(x.get("whale_buy_count")) + num(x.get("whale_sell_count")),
        }
    return {}
...
def wallet_snapshot(md,meta):
    w={"wallet":WALLET_ADDRESS,"native_sda":None,"holdings":[],"total_token_value_sda":0.0,"total_value_sda":None,"updated_at":now(),"error":None,"token_balance_items":0}
    try:
    ...
    try:
        ...
        # Blockscout versions differ: token address can be address/address_hash,
        # decimals can be on token or balance, and the payload can be list/items/data.
        if isinstance(d, list):
            bs = d
        elif isinstance(d, dict):
            bs = d.get("items") or d.get("data") or d.get("result") or []
        else:
            bs = []

        if isinstance(bs, dict):
            bs = list(bs.values())
        w["token_balance_items"] = len(bs)

        for b in bs:
            if not isinstance(b, dict):
                continue
            t = b.get("token") or b.get("token_info") or {}

            a = str(
                t.get("address")
                or t.get("address_hash")
                or t.get("contract_address")
                or b.get("token_address")
                or b.get("token_address_hash")
                or b.get("address")
                or ""
            ).lower()
            if not a or not a.startswith("0x"):
                continue

            raw_dec = t.get("decimals") or b.get("decimals") or 18
            try:
                dec = int(raw_dec)
            except Exception:
                dec = 18
            ...
            raw = (
                b.get("value")
                if b.get("value") is not None else
                b.get("balance")
                if b.get("balance") is not None else
                b.get("token_balance")
                if b.get("token_balance") is not None else
                b.get("amount")
            )
            if raw is None:
                continue
            ...
            try:
                # Blockscout can return a decimal string already scaled, but normally
                # returns the raw integer token balance. Use the declared decimals.
