...
Learns from completed paper trades and missed opportunities without changing
BUY, SL, TP or exits. The objective is expected post-fee P/L, while MFE
thresholds are tracked separately so large pump paths are not lost.
"""
...
def _build_samples(closed: Iterable[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    for tr in _complete(closed):
        ...
        out.append({"metrics": m, "profit_sda": tr["profit"], "mfe": _num(tr["mfe"]), "mae": _num(tr["mae"]), "times": tr["times"]})
    return out
...
def predict(metrics: Dict[str, Any], closed_trades: Iterable[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    if coverage < MIN_SAMPLES:
        return {"version":"V25","ready":False,"samples":coverage,"neighbors":0,"expected_pl_sda":0.0,"p5":0.0,"p10":0.0,"p20":0.0,"p30":0.0,"expected_mfe":0.0,"expected_mae":0.0,"learning_mode":"shadow"}
    ...
    return {
        "version":"V25","ready":True,"samples":coverage,"neighbors":len(ranked),
        "expected_pl_sda":round(wavg("profit_sda"),6),
...
def learn(closed_trades: Iterable[Dict[str, Any]]) -> Dict[str, Any]:
    samples = _build_samples(closed_trades)
    state = _load()
    state.update({"version":"V25","updated_at":datetime.now(timezone.utc).isoformat(),"samples":len(samples),"mode":"shadow","objective":"expected_post_fee_pnl"})
    _save(state)
    return state
